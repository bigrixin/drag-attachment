...
namespace MyAbilityFirst.Controllers
{
	...
	public class CareWorkerController : Controller
	{
		...
		[HttpGet, Route("careworker/{careworkerID:int}/attachments/")]
		public ActionResult ManageAttachments(int careWorkerID)
		{
			var carer = this.GetLoggedInUser() as CareWorker;
			var currentCarer = this._careWorkerServices.RetrieveCareWorker(careWorkerID);
			if (currentCarer != null && carer.ID==careWorkerID)
			{
				CareWorkerAttachmentViewModel vm = _mapper.Map<CareWorker, CareWorkerAttachmentViewModel>(carer);
				ViewBag.PathUpload = Url.Action("Upload", "Attachment");
				ViewBag.PathDelete = Url.Action("Delete", "Attachment");
				return View(vm);
			}
			return RedirectToAction("MyAccount", "CareWorker");
		}
