		...
		$('.fileupload').each(function () {
			$(this).fileupload({
				...
				add: function (e, data) {
					...
					var uploadErrors = [];
					var fileType = data.files[0].name.split('.').pop(), allowdtypes = 'jpeg,jpg,png,gif,pdf,doc,docx,xls,xlsx,JPEG,JPG,PNG,GIF,PDF,DOC,DOCX,XLS,XLSX';
				...
				fail: function (event, data) {
					...
					if (data.result.statusCode === 200) {
						...
						$('#' + barId).css('width', '0%');
						$('#' + barId).text('0 %');
						alert("Error: " + data.result.status);
					}
