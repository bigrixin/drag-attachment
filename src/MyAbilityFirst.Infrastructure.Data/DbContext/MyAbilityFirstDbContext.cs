	...
	public sealed class MyAbilityFirstDbContext : DbContext, IWriteEntities
	{
		...
		protected override void OnModelCreating(DbModelBuilder modelBuilder)
		{
			...
			modelBuilder.Entity<UserAttachment>()
										...
										.Map<CV>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.CV))
										.Map<Qualifications>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.Qualifications))
										.Map<Referencees>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.Referencees))
										.Map<SpecialAwards>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.SpecialAwards))
										.Map<DriverLicense>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.DriverLicense))
										.Map<RegisteredVehicle>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.RegisteredVehicle))
										.Map<CPRAndFirstAidCertificate>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.CPRAndFirstAidCertificate))
										.Map<PoliceCriminalRecordsCheck>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.PoliceCriminalRecordsCheck))
										.Map<Passport>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.Passport))
										.Map<WorkingWithChildrenCheck>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.WorkingWithChildrenCheck))
										.Map<WorkingWithVulnerablePeopleCheck>(m => m.Requires("AttachmentTypeID").HasValue((int)AttachmentType.WorkingWithVulnerablePeopleCheck));
