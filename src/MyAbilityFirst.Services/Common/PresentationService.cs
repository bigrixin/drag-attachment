...
namespace MyAbilityFirst.Services.Common
{
	public class PresentationService : IPresentationService
	{
		...
		public List<AttachmentType> GetPatientAttachmentList()
		{
			IEnumerable<AttachmentType> res = Enum.GetValues(typeof(AttachmentType)).Cast<AttachmentType>().Where(a => ((int)a < 2000));
			return res.ToList();
		}

		public List<AttachmentType> GetCarerAttachmentList()
		{
			IEnumerable<AttachmentType> res = Enum.GetValues(typeof(AttachmentType)).Cast<AttachmentType>().Where(a => ((int)a >= 2000));
			return res.ToList();
		}
